...
<body>
    ...
    <script>
        ...
        const BRANCH = 'main';
        const TOKEN_KEY = 'gh_steps_token';
        ...
        function promptToken() {{
            const token = prompt(
                'Para enviar a planilha ao repositório, informe seu GitHub Personal Access Token (classic) com permissão "repo".\\n\\n' +
        ...
        async function uploadToGitHub(file) {{
            ...
            if (!token) {{
                token = promptToken();
                if (!token) {{
                    showToast('❌ Upload cancelado — token não fornecido.', 'error');
                    return false;
                }}
            }}

            showToast('⏳ Enviando planilha ao repositório...', 'info');
            ...
            try {{
                ...
                const body = {{
                    message: 'Update step-tracking spreadsheet',
                    content: base64,
                    branch: BRANCH
                }};
                if (sha) body.sha = sha;
                ...
                if (resp.ok) {{
                    showToast('✅ Planilha enviada! O dashboard será atualizado automaticamente em instantes.', 'success');
                    return true;
                }} else {{
                    const err = await resp.json();
                    if (resp.status === 401 || resp.status === 403) {{
                        localStorage.removeItem(TOKEN_KEY);
                        showToast('❌ Token inválido ou sem permissão. Tente novamente no próximo upload.', 'error');
                    }} else {{
                        showToast('❌ Erro ao enviar: ' + (err.message || resp.statusText), 'error');
                    }}
                    return false;
                }}
            }} catch (err) {{
