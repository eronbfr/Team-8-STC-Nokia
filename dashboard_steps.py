...
<head>
    ...
    <style>
        ...
        * {{
        ...
        .token-modal-buttons .btn-cancel {{
            background: #e5e5e5;
            color: #333;
        }}
        .token-modal-buttons .btn-save {{
            background: linear-gradient(135deg, #00C48C 0%, #009DE0 100%);
            color: #fff;
        }}
    </style>
</head>
<body>
    ...
    <div id="tokenModalOverlay" class="token-modal-overlay">
        <div class="token-modal">
            ...
            <div class="token-modal-buttons">
                ...
                <button class="btn-save" onclick="confirmToken()">Save &amp; Upload</button>
            </div>
        </div>
    </div>
    <div class="dashboard">
    ...
    <script>
        ...
        const REPO_OWNER = 'eronbfr';
        const REPO_NAME = 'Team-8-STC-Nokia';
        ...
        function getToken() {{
            return localStorage.getItem('gh_token_stc') || null;
        }}

        function promptForToken() {{
            document.getElementById('tokenModalOverlay').classList.add('show');
            document.getElementById('tokenInput').value = '';
            document.getElementById('tokenInput').focus();
        }}

        function cancelToken() {{
            document.getElementById('tokenModalOverlay').classList.remove('show');
            pendingFileBase64 = null;
            showToast('Upload cancelled — no token provided.', 'info');
        }}

        function confirmToken() {{
            const val = document.getElementById('tokenInput').value.trim();
            if (!val) return;
            localStorage.setItem('gh_token_stc', val);
            document.getElementById('tokenModalOverlay').classList.remove('show');
            if (pendingFileBase64) {{
                pushToRepo(pendingFileBase64);
                pendingFileBase64 = null;
            }}
        }}

        async function pushToRepo(base64Content) {{
            const token = getToken();
            if (!token) {{
                pendingFileBase64 = base64Content;
                promptForToken();
                return;
            }}
            showToast('⏳ Uploading to repository…', 'info');
            try {{
                ...
                if (getResp.ok) {{
                    ...
                    localStorage.removeItem('gh_token_stc');
                    showToast('❌ Invalid or expired token. Please try uploading again.', 'error');
                    return;
                }}
                ...
                if (putResp.ok) {{
                    showToast('✅ Spreadsheet saved to repository! Dashboard will update automatically.', 'success');
                }} else if (putResp.status === 401 || putResp.status === 403) {{
                    localStorage.removeItem('gh_token_stc');
                    showToast('❌ Token rejected. Please try uploading again.', 'error');
                }} else {{
                    const err = await putResp.json();
                    showToast('❌ Upload failed: ' + (err.message || putResp.status), 'error');
                }}
            }} catch (err) {{
        ...
        function handleUpload(input) {{
            ...
            base64Reader.onload = function(ev) {{
                const base64 = ev.target.result.split(',')[1];
                // Push to repo automatically
                pushToRepo(base64);
            }};
            base64Reader.readAsDataURL(file);

            // Also read as ArrayBuffer for local dashboard update
            ...
            reader.onload = function(e) {{
                ...
                try {{
                    ...
                    for (let r = 2; r < Math.min(12, data.length); r++) {{
                        ...
                        // Find matching existing member for merge
                        const existingMember = memberData.find(m => m.name === row[1]);
                        const daily = [];
                        let total = 0;
                    ...
                    showToast('✅ Spreadsheet loaded locally!', 'success');
                }} catch (err) {{
